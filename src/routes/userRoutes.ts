...
userRouter.post("/", async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (nameParts.length < 2) {
        res.status(400).json({ error: "O nome completo deve incluir pelo menos um primeiro e um último nome" });
        return;
      }
      ...
      const lastUser = await userRepo.createQueryBuilder("user").orderBy("user.id", "DESC").getOne();
